...
You can manually download FidelSDK.aar and import it as a new .jar/.aar module with _File / New Module / Import .JAR/.AAR package_ command.
...
If you use the manual installation, you also need to add the following dependencies:
```java
...
Notice the following excerpt from the consent text above: _in accordance with the `Cashback Inc` Terms and Conditions, `Cashback Inc` privacy policy and Fidel’s Privacy Policy._ If you set a `privacyURL`, this is the text that will be displayed, along with a hyperlink set on _privacy policy_.
...
- `STRING_OVER_THE_LIMIT` - We send this error in case your `Fidel.deleteInstructions` or `Fidel.companyName` or `Fidel.programName` exceed _60_ characters.
